...
class Config:
    def __init__(self, ds_name='ycb', cls_type=''):
        ...
        self.mini_batch_size = 7
        self.val_mini_batch_size = 7
        self.test_mini_batch_size = 1

        self.n_sample_points = 480 * 640 // 24  # Number of input points
        self.n_keypoints = 8
        self.n_edges = self.n_keypoints * (self.n_keypoints - 1) // 2
        ...
        if self.dataset_name == 'ycb':
        ...
        elif self.dataset_name == 'boplm':
            self.n_objects = 15 + 1  # 8 objects + background
            self.n_classes = self.n_objects

            self.use_orbfps = True
            self.kp_orbfps_dir = os.path.join(self.exp_dir, 'datasets/lmo/kps_orb9_fps/')
            self.kp_orbfps_ptn = os.path.join(self.kp_orbfps_dir, '%s_%d_kps.txt')

            self.boplm_root = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/lmo'
                )
            )
            self.boplm_kps_dir = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/lmo/kps_orb9_fps/'
                )
            )

            self.boplm_cls_lst_p = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/lmo/dataset_config/classes.txt'
                )
            )

            self.boplm_cls_lst = self.read_lines(self.boplm_cls_lst_p)
            self.boplm_sym_cls_ids = [3, 10, 11]

            boplm_r_pth = os.path.join(self.exp_dir, 'datasets/lmo/dataset_config/models_info.json')
            with open(os.path.join(boplm_r_pth), "r") as f:
                lm_r_dict = json.load(f)
            self.boplm_r_lst = [v['diameter']/1000. for v in lm_r_dict.values()]
        else:  # linemod
            ...
            self.kp_orbfps_dir = os.path.abspath(
                os.path.join(self.exp_dir, 'datasets/linemod/kps_orb9_fps')
            )
            self.kp_orbfps_ptn = os.path.join(self.kp_orbfps_dir, '%s_%d_kps.txt')
            # FPS
