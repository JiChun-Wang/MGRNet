...
class PoseTransformer(object):
    ...
    def __init__(self, class_type):
        ...
        lm_pth = '/home/ubuntu/Code/Python/mypose/ffb6d/datasets/linemod/LINEMOD'
        lm_occ_pth = '/home/ubuntu/Code/Python/mypose/ffb6d/datasets/linemod/Occlusion_linemod'
...
class Basic_Utils():
    ...
    def __init__(self, config):
        ...
        elif config.dataset_name == 'boplm':
            self.boplm_cls_lst = config.boplm_cls_lst
        self.ycb_cls_ptsxyz_dict = {}
        self.ycb_cls_ptsxyz_cuda_dict = {}
        self.ycb_cls_kps_dict = {}
        self.ycb_cls_ctr_dict = {}
        self.boplm_cls_ptsxyz_dict = {}
        self.boplm_cls_ptsxyz_cuda_dict = {}
        self.boplm_cls_kps_dict = {}
        self.boplm_cls_ctr_dict = {}
    ...
    def get_pointxyz(
        ...
        if ds_type == "ycb":
            ...
            cls = self.get_cls_name(cls, ds_type)
            if cls in self.boplm_cls_ptsxyz_dict.keys():
                return self.boplm_cls_ptsxyz_dict[cls]
            cls_id = self.config.boplm_cls_lst.index(cls) + 1
            ptxyz_pth = os.path.join(
                self.config.boplm_root, 'models_eval/obj_{:06d}.ply'.format(cls_id),
            )
            pointxyz = self.ply_vtx(ptxyz_pth) / 1000.0
            self.boplm_cls_ptsxyz_dict[cls] = pointxyz
            return pointxyz
        else:
            ptxyz_pth = os.path.join(
    ...
    def get_pointxyz_cuda(
        ...
        if ds_type == "ycb":
            ...
            if cls in self.boplm_cls_ptsxyz_cuda_dict.keys():
                return self.boplm_cls_ptsxyz_cuda_dict[cls].clone()
            ptsxyz = self.get_pointxyz(cls, ds_type)
            ptsxyz_cu = torch.from_numpy(ptsxyz.astype(np.float32)).cuda()
            self.boplm_cls_ptsxyz_cuda_dict[cls] = ptsxyz_cu
            return ptsxyz_cu.clone()
        else:
    ...
    def get_kps(
        ...
        if ds_type == "ycb":
            ...
            if cls in self.boplm_cls_kps_dict.keys():
                return self.boplm_cls_kps_dict[cls].copy()
            if use_orbfps:
                kps_pth = self.config.kp_orbfps_ptn % (cls, self.config.n_keypoints)
            else:
                kps_pth = os.path.join(
                    self.config.boplm_kps_dir, '{}/{}.txt'.format(cls, kp_type)
                )
            kps = np.loadtxt(kps_pth, dtype=np.float32)
            self.boplm_cls_kps_dict[cls] = kps
        else:
            ...
            if use_orbfps:
                ...
                kps_pattern = os.path.join(
                    self.config.kp_orbfps_dir, "{}/{}.txt".format(cls, kp_type)
                )
    ...
    def get_ctr(self, cls, ds_type='ycb', ctr_pth=None):
        ...
        if ds_type == "ycb":
            ...
            if cls in self.boplm_cls_ctr_dict.keys():
                return self.boplm_cls_ctr_dict[cls].copy()
            cor_pattern = os.path.join(
                self.config.kp_orbfps_dir, '{}_corners.txt'.format(cls),
            )
            cors = np.loadtxt(cor_pattern.format(cls), dtype=np.float32)
            ctr = cors.mean(0)
            self.boplm_cls_ctr_dict[cls] = ctr
        else:
    ...
    def calc_cov_cuda(self, A):
        N, c = A.size()
        if N < 2:
            return torch.eye(c).cuda()
        else:
            mean = torch.mean(A.t(), dim=1)
            centrA = A - mean
            cov_mat = torch.matmul(centrA.t(), centrA) / (N - 1)
            return cov_mat
