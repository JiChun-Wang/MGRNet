...
class Dataset():
    def __init__(self, cls_type="duck", DEBUG=False):
        ...
        self.occlinemod_root = os.path.join(self.config.lm_root, 'Occlusion_linemod')
        self.linemod_root = os.path.join(self.config.lm_root, 'Linemod_preprocessed')

        self.real_lst = list(filter(lambda x: x.endswith('txt'),
                                    os.listdir(os.path.join(self.occlinemod_root, 'valid_poses/{}'.format(self.cls_type)))))

        self.rng = np.random
        meta_file = open(os.path.join(self.linemod_root, 'data/02', 'gt.yml'), "r")
        self.meta_lst = yaml.load(meta_file, Loader=yaml.FullLoader)
        print("occlinemod_dataset_size: {}".format(len(self.real_lst)))

    def read_pose_and_img_id(self, filename):
        read_rotation = False
    ...
    def get_item(self, item):
        sample_pth = os.path.join(self.occlinemod_root, 'valid_poses', '{}'.format(self.cls_type), item)
        R, t, img_id = self.read_pose_and_img_id(sample_pth)
        # image
        with Image.open(os.path.join(self.occlinemod_root, 'RGB-D/rgb_noseg/color_{:05d}.png'.format(img_id))) as ri:
            rgb = np.array(ri)[:, :, :3]
        # depth
        with Image.open(os.path.join(self.occlinemod_root, 'RGB-D/depth_noseg/depth_{:05d}.png'.format(img_id))) as di:
            dpt_mm = np.array(di)
        # mask
        with Image.open(os.path.join(self.occlinemod_root, 'masks/{}/{}.png'.format(self.cls_type, img_id))) as li:
            labels = np.array(li)
            labels = (labels > 0).astype(np.uint8)
        # RT
        meta = self.meta_lst[img_id]
        for i in range(0, len(meta)):
            if meta[i]['obj_id'] == self.cls_id:
                meta = meta[i]
                break
