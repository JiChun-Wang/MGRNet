...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="duck", DEBUG=False):
        ...
        self.normal = self.read_normal(os.path.join(self.root, 'symmetries', cls_type, 'symmetries.txt'))
        ...
        elif dataset_name == 'trainval':
            self.add_noise = False
            real_img_pth = os.path.join(
                self.cls_root, "train.txt"
            )
            self.all_lst = self.bs_utils.read_lines(real_img_pth)
        else:
            self.add_noise = False
    ...
    def get_item(self, item_name):
        if "pkl" in item_name:
            ...
            rgb = data['rgb']
            labels = data['mask']
            example_id = int(item_name.split('/')[-1].split('.')[0])
            K = data['K']
            RT = data['RT']
            rnd_typ = data['rnd_typ']
            if rnd_typ == "fuse":
                labels = (labels == self.cls_id).astype("uint8")
                sym_cor = np.load(os.path.join(self.root,
                                               'fuse/{}_labels/cor{}.npz'.format(self.cls_type, example_id)))['cor']
            else:
                labels = (labels > 0).astype("uint8")
                sym_cor = np.load(os.path.join(self.root,
                                               'renders/{}_labels/cor{}.npz'.format(self.cls_type, example_id)))['cor']
        else:
            ...
            with Image.open(os.path.join(self.cls_root, "rgb/{}.png".format(item_name))) as ri:
                ...
                rgb = np.array(ri)[:, :, :3]
            sym_cor = np.load(os.path.join(self.root,
                                           'data/{:02d}_labels/cor{}.npz'.format(self.cls_id, int(item_name))))['cor']
        ...
        labels_pt = labels.flatten()[choose]                                                # [n_pts,]
        choose = np.array([choose])                                                         # [n_pts,]
        cld_rgb_nrm = np.concatenate((cld, rgb_pt, nrm_pt), axis=1).transpose(1, 0)         # [9, n_pts]
    ...
    def get_pose_gt_info(self, cld, labels, RT):          # cld: [n_pts, 3]  labels: [n_pts,]
        ...
        edg_targ_ofst = np.zeros((self.config.n_sample_points, self.config.n_edges, 3))
        for i, cls_id in enumerate([1]):
            ...
            edge_idx = 0
            for start_idx in range(0, self.config.n_keypoints - 1):
                start = kps[start_idx]
                for end_idx in range(start_idx + 1, self.config.n_keypoints):
                    end = kps[end_idx]
                    edge = end - start
