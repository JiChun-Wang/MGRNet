...
void PoseRegression::GenerateDataMatrix(const HybridPredictionContainer& predictions,
  ...
  Matrix<double, 12, Dynamic> J_kpts(12, numKpts*3);
  Matrix<double, Dynamic, Dynamic> W_kpts(numKpts*3, numKpts*3);
  ...
  for (unsigned ptId = 0; ptId < numKpts; ++ptId) {  
    ...
    x_hc = kp.point3D_cam[0];  
    y_hc = kp.point3D_cam[1]; 
    z_hc = kp.point3D_cam[2];

    double x_3d, y_3d, z_3d;
    x_3d = kp.point3D_obj[0];
    y_3d = kp.point3D_obj[1];
    z_3d = kp.point3D_obj[2];
  ...
  for (unsigned edgeId = 0; edgeId < numedges; ++edgeId) {
    ...
    x_hc = kp_end.point3D_cam[0];  
    y_hc = kp_end.point3D_cam[1]; 
    z_hc = kp_end.point3D_cam[2];

    double x_3d, y_3d, z_3d; // denotes edge3d
    x_3d = kp_start.point3D_obj[0] - kp_end.point3D_obj[0];
    y_3d = kp_start.point3D_obj[1] - kp_end.point3D_obj[1];
    z_3d = kp_start.point3D_obj[2] - kp_end.point3D_obj[2];
    ...
    x_3d = kp_start.point3D_obj[0];// denotes pts3d_t
    y_3d = kp_start.point3D_obj[1];
    z_3d = kp_start.point3D_obj[2];
...
void PoseRegression::LeadingEigenSpace(Matrix12d& data_matrix, const unsigned& numEigs, 
 ...
 // check2 depth > 0
  ...
  for (unsigned ptId = 0; ptId < numKpts; ++ptId) {  
    const Keypoint& kp = (*keypoints)[ptId];
    z = z + (R_init * kp.point3D_obj + t_init)[2];
  }
