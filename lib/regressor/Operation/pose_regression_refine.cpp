...
void PoseRegression::RefinePose(const HybridPredictionContainer& predictions,
  ...
  projected_keypoints_1st_order_z.resize(keypoints->size());
    ...
    for (unsigned innerIter = 0; innerIter < para.numGaussNewtonIters; ++innerIter) {
      ...
      for (unsigned ptId = 0; ptId < keypoints->size(); ++ptId)
        Projection_jacobi((*keypoints)[ptId], *rigid_pose,
          ...
          &projected_keypoints_1st_order_z[ptId]);
      ...
      for (unsigned ptId = 0; ptId < keypoints->size(); ++ptId) {
        ...
        Vector3d adjusted_dif = kp.inv_half_var * (kp.point3D_cam - projected_keypoints[ptId]);

        const Vector6d& jacobi_x = projected_keypoints_1st_order_x[ptId];
        const Vector6d& jacobi_y = projected_keypoints_1st_order_y[ptId];
        const Vector6d& jacobi_z = projected_keypoints_1st_order_z[ptId];
        Vector6d adjusted_jacobi_x = jacobi_x * kp.inv_half_var(0, 0) + jacobi_y * kp.inv_half_var(0, 1) 
                                    + jacobi_z * kp.inv_half_var(0, 2); 
        Vector6d adjusted_jacobi_y = jacobi_x * kp.inv_half_var(1, 0) + jacobi_y * kp.inv_half_var(1, 1) 
                                    + jacobi_z * kp.inv_half_var(1, 2);
        Vector6d adjusted_jacobi_z = jacobi_x * kp.inv_half_var(2, 0) + jacobi_y * kp.inv_half_var(2, 1) 
                                    + jacobi_z * kp.inv_half_var(2, 2);
        ...
        b += adjusted_jacobi_x * (adjusted_dif[0] * w) + adjusted_jacobi_y * (adjusted_dif[1] * w) 
            + adjusted_jacobi_z * (adjusted_dif[2] * w);

        // U pdate second-order approximation
        for (int i = 0; i < 6; ++i)
          for (int j = i; j < 6; ++j)
            A(i, j) += (adjusted_jacobi_x[i] * adjusted_jacobi_x[j]
              + adjusted_jacobi_y[i] * adjusted_jacobi_y[j]
              + adjusted_jacobi_z[i] * adjusted_jacobi_z[j]) * w;
      }
      for (unsigned edgeId = 0; edgeId < edges->size(); ++edgeId) {
        const EdgeVector& ev = (*edges)[edgeId];
        Vector3d dif = ev.vec_pred - (projected_keypoints[ev.start_id] - projected_keypoints[ev.end_id]);
        Vector3d adjusted_dif = ev.inv_half_var * dif;
        ...
        Vector6d jacobi_edge_z = projected_keypoints_1st_order_z[ev.start_id]
          - projected_keypoints_1st_order_z[ev.end_id];
        Vector6d adjusted_jacobi_edge_x = jacobi_edge_x * ev.inv_half_var(0, 0)
          + jacobi_edge_y * ev.inv_half_var(0, 1) + jacobi_edge_z * ev.inv_half_var(0, 2);
        Vector6d adjusted_jacobi_edge_y = jacobi_edge_x * ev.inv_half_var(1, 0)
          + jacobi_edge_y * ev.inv_half_var(1, 1) + jacobi_edge_z * ev.inv_half_var(1, 2);
        Vector6d adjusted_jacobi_edge_z = jacobi_edge_x * ev.inv_half_var(2, 0)
          + jacobi_edge_y * ev.inv_half_var(2, 1) + jacobi_edge_z * ev.inv_half_var(2, 2);
        ...
        b += adjusted_jacobi_edge_x * (adjusted_dif[0] * w) + adjusted_jacobi_edge_y * (adjusted_dif[1] * w)
            + adjusted_jacobi_edge_z * (adjusted_dif[2] * w);

        // Update the second-order approximation
        for (int i = 0; i < 6; ++i)
          for (int j = i; j < 6; ++j)
            A(i, j) += (adjusted_jacobi_edge_x[i] * adjusted_jacobi_edge_x[j]
              + adjusted_jacobi_edge_y[i] * adjusted_jacobi_edge_y[j]
              + adjusted_jacobi_edge_z[i] * adjusted_jacobi_edge_z[j]) * w;
      }
...
void PoseRegression::Reweighting(const HybridPredictionContainer& predictions,
  ...
  vector<Vector3d> buffer_projections;
  ...
  for (unsigned id = 0; id < buffer_projections.size(); ++id) {
    ...
    Vector3d dif = kp.point3D_cam - buffer_projections[id];
    Vector3d adjusted_dif = kp.inv_half_var * dif;  
  ...
  for (unsigned id = 0; id < edges->size(); ++id) {
    ...
    Vector3d dif = ev.vec_pred - (buffer_projections[ev.start_id] - buffer_projections[ev.end_id]);
    Vector3d adjusted_dif = ev.inv_half_var * dif;
...
void PoseRegression::Projection(const Keypoint& pt, const AffineXform3d& aff,
  Vector3d* p_cur) {
  *p_cur = aff[0] + aff[1] * pt.point3D_obj[0]
    + aff[2] * pt.point3D_obj[1] + aff[3] * pt.point3D_obj[2];
}

// Compute the Jacobi of the keypoint term
void PoseRegression::Projection_jacobi(const Keypoint& pt, const AffineXform3d& aff,
  Vector3d* p_cur, Vector6d* jacobi_x, Vector6d* jacobi_y, Vector6d* jacobi_z) {
  Vector3d pos_camera = aff[0] + aff[1] * pt.point3D_obj[0]
    + aff[2] * pt.point3D_obj[1] + aff[3] * pt.point3D_obj[2];
  *p_cur = pos_camera;
  ...
  (*jacobi_x)[2] = 0;
  (*jacobi_x)[3] = 0;
  (*jacobi_x)[4] = pos_camera[2];
  (*jacobi_x)[5] = -pos_camera[1];

  (*jacobi_y)[0] = 0;
  (*jacobi_y)[1] = 1;
  (*jacobi_y)[2] = 0;
  (*jacobi_y)[3] = -pos_camera[2];
  (*jacobi_y)[4] = 0;
  ...
  (*jacobi_z)[1] = 0;
  (*jacobi_z)[2] = 1;
  (*jacobi_z)[3] = pos_camera[1];
  (*jacobi_z)[4] = -pos_camera[0];
  (*jacobi_z)[5] = 0;
}
