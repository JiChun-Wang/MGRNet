...
class FFB6D(nn.Module):
    def __init__(
        ...
        self.kp_ofst_layer = (
            ...
            .conv1d(128, bn=True, activation=nn.ReLU())
            .conv1d(128, bn=True, activation=nn.ReLU())
            .conv1d((self.n_kps+self.n_edges)*3+3, activation=None)
        )
        # self.kp_ofst_layer = (
        #     pt_utils.Seq(self.up_rndla_oc[-1] + self.up_rgb_oc[-1])
        #         .conv1d(128, bn=True, activation=nn.ReLU())
    ...
    def forward(
        ...
        pred_kp_ofs = pred_kp_ofs.view(
            bs, self.n_kps+self.n_edges+1, 3, -1
        ...
        end_points['pred_sym_cor'] = pred_kp_ofs[:, :1]
        end_points['pred_kp_ofs'] = pred_kp_ofs[:, 1:-self.n_edges]
        end_points['pred_graph'] = pred_kp_ofs[:, -self.n_edges:]
